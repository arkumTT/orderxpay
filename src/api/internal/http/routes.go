...
func registerMerchantScopedRoutes(r fiber.Router, h *handlers.Handler) {
	...
	own.Get("/analytics", h.GetMerchantAnalytics)
	own.Get("/records/export", h.ExportMerchantRecords)
	...
	own.Patch("/fee-rule", h.SetMerchantOwnFeeRule)
	own.Patch("/whatsapp-settings", h.UpdateMerchantWhatsAppSettings)
