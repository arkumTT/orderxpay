...
// merchant list is already loaded on the page, no new endpoint needed.
export function CsvExportButton({ merchants }: { merchants: Merchant[] }) {
  function handleExport() {
    const lines = [
      ["Business name", "Category", "Phone", "KYC tier", "Status", "Created at"],
      ...merchants.map((m) => [
        m.business_name,
        m.category ?? "",
        m.phone,
    ...
    a.href = url;
    a.download = `orderxpay-merchants-${isoDate(new Date())}.csv`;
    a.click();
